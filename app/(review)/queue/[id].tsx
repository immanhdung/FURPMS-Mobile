...
export default function CouncilWorkspaceScreen() {
  ...
  const tabs: { key: Tab; label: string }[] = [
    ...
    ...(!isSecretary ? [{ key: 'SCORING' as Tab, label: 'Scoring' }] : []),
    ...(isAcceptanceRound ? [{ key: 'ACCEPTANCE' as Tab, label: 'Acceptance' }] : []),
  ...
  if (isLoading) return <LoadingState message="Loading council…" />;
  if (isError) return <ErrorState title="Could not load council" message="Check your connection and try again." onRetry={refetch} />;
  if (!membership) return <ErrorState title="Not found" message="This council isn't part of your reviews." onRetry={refetch} />;
          ...
          <View className="px-5 mb-4 gap-2">
            {meetings.map((m) => (
              <View key={m.id} className="bg-white dark:bg-dark-50 rounded-2xl border border-neutral-100 dark:border-dark-200 p-4 gap-1.5">
                <Text className="text-neutral-900 dark:text-neutral-50 text-sm font-semibold">{m.title || 'Council meeting'}</Text>
                ...
                {m.meetingLink && (
                  <TouchableOpacity onPress={() => Linking.openURL(m.meetingLink!)} activeOpacity={0.7} className="flex-row items-center gap-1.5 mt-1">
                    ...
                    <Text className="text-violet-600 dark:text-violet-400 text-xs font-medium">Join meeting</Text>
                  </TouchableOpacity>
                )}
              </View>
            ))}
